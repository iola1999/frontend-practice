...
[https://mp.weixin.qq.com/s/GLxlZ4hptmOLd711ZtyGXg](https://mp.weixin.qq.com/s/GLxlZ4hptmOLd711ZtyGXg)

### 先了解原理

通过 3 个步骤，实现对 total 数据进行响应式变化：
...
> 一定要四次挥手？？ 第二次和第三次 合并在一起不行嘛 ？？？

> 不行的， 因为客户端 已经做好了 结束连接的准备了， 但是服务器端不一定已经准备好了，可能还有一些报文 要继续发送， 所以服务器端也准备好了， 再去发送一个 FIN =1 的一个报文告诉 客户端 我已经准备好了。
...
### Promise .then 里面出错了，会被 .catch 捕获？
...
Promise.resolve(0).then(res => {
  ...
  console.log('1', err)   // 1 ReferenceError: b is not defined
})
```
...
### getBoundingClientRect 相对于？尺寸来源？
...
一般来说 sw 要设置不缓存，所以还可以给用户以提示：

```javascript
function emitUpdate() {
  var event = document.createEvent('Event');
  event.initEvent('sw.update', true, true);
  window.dispatchEvent(event);  // 然后前端页面弹 notify
}
...
在被动关闭连接情况下，在已经接收到FIN，但是还没有发送自己的FIN的时刻，连接处于CLOSE_WAIT状态。 如果我们的服务器程序处于CLOSE_WAIT状态的话，说明套接字是被动关闭的

出现大量close_wait的现象，主要原因是某种情况下对方关闭了socket链接，但是我方忙与读或者写，没有关闭连接。

### UDP 特点

面向报文，无连接，不可靠交付，无流量控制（滑动窗口），无拥塞控制，首部开销小，不保证数据顺序，无握手
